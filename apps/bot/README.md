...
Exit code `2` is an intentional safety stop: `RestartPreventExitStatus=2` leaves the unit failed instead of restarting it, and `systemctl --user --failed` lists it. It does not survive a reboot: an enabled unit starts again with the user manager. Before restarting, inspect the journal for the terminal event and the exit status:
