...
export async function writeBotArtifact(options: {
  ...
  const outputDir = join(options.artifactRoot, kindDir);
...
async function ensureArtifactDirectory(outputDir: string): Promise<void> {
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- Artifact root is an explicit runtime output directory; kind/file names are deterministic below it.
...
async function writeContentAddressedArtifact(
  ...
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- Opens only a deterministic hash file sibling temp under the configured artifact directory.
    ...
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- Links the completed temp file to the deterministic content-addressed path without overwrite.
...
async function verifyExistingArtifact(path: string, expectedHash: string): Promise<void> {
  let handle: FileHandle | undefined;
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- Verifies only the deterministic hash file under the configured artifact directory.
...
async function assertNoSymlinkedArtifactPath(path: string): Promise<void> {
  ...
  for (const part of parts) {
    ...
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Checks configured artifact path components before writing artifacts.
