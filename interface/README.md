...
Every merge to the default branch deploys this bundle to GitHub Pages at ickb.org: the check workflow uploads the `interface/dist` it just built and a deploy job publishes it once the gate passed on the same commit, so a failed run leaves the previous deployment live. The `CNAME` in `public/` binds the domain; the repository's Pages source is GitHub Actions.
