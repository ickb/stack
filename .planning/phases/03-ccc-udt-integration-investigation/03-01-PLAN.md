...
must_haves:
  ...
  key_links:
    - from: "ccc-fork/ccc/packages/udt/src/udt/index.ts"
...
1. **Udt class and infoFrom** (`ccc-fork/ccc/packages/udt/src/udt/index.ts`):
...
2. **CellAny and Cell types** (`ccc-fork/ccc/packages/core/src/ckb/transaction.ts`):
...
3. **UdtInfo type** (`ccc-fork/ccc/packages/udt/src/udt/index.ts` or related):
...
4. **Header access** (`ccc-fork/ccc/packages/core/src/client/client.ts`):
...
5. **PR #328 compatibility check** (`ccc-fork/ccc/packages/core/src/signer/feePayer/`):
