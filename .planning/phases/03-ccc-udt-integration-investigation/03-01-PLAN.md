...
must_haves:
  truths:
    ...
    - "All open questions from 03-RESEARCH.md (capacityFree on CellAny, filter mismatch, PR #328 compatibility) are answered with code evidence"
  ...
  key_links:
    - from: "ccc-dev/ccc/packages/udt/src/udt/index.ts"
...
1. **Udt class and infoFrom** (`ccc-dev/ccc/packages/udt/src/udt/index.ts`):
...
2. **CellAny and Cell types** (`ccc-dev/ccc/packages/core/src/ckb/transaction.ts`):
   - Verify `CellAny.outPoint` is `OutPoint | undefined` (not nullable)
   - Verify `CellAny` vs `Cell` -- does `CellAny` have `capacityFree`? If not, what does?
...
3. **UdtInfo type** (`ccc-dev/ccc/packages/udt/src/udt/index.ts` or related):
...
4. **Header access** (`ccc-dev/ccc/packages/core/src/client/client.ts`):
...
5. **PR #328 compatibility check** (`reference/ccc-fee-payer/`):
