...
- `ccc-dev/pins/` is **committed** to git (manifest + unified diffs + local patches), regenerated by `pnpm ccc:record`
  - `pins/HEAD` — expected final SHA after full replay
  - `pins/manifest` — base SHA + merge refs (TSV, one per line)
  - `pins/res-N.diff` — conflict resolution for merge step N (standard unified diff)
