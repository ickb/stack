...
- `ccc-dev/pins/` is **committed** to git (single SHA-named file containing base SHA, merge refs, and conflict resolutions), regenerated by `pnpm ccc:record`
...
- `pnpm install` has a side effect: if `ccc-dev/pins/` contains a SHA-named file but `ccc-dev/ccc/` does not, it automatically runs `ccc-dev/replay.sh` to rebuild CCC from pins. This is intentional
- `ccc-dev/patch.sh` rewrites CCC package exports to point at `.ts` source instead of `.d.ts`, then creates a deterministic git commit (fixed author/date) so record and replay produce the same HEAD hash (used as the pins filename). This is why imports from `@ckb-ccc/*` resolve to TypeScript source files inside `node_modules` — it is not a bug
...
Once the PR is open, add the PR number to `ccc:record` in `package.json` — order PRs by target branch from upstream to downstream, so each group merges cleanly onto its base before the next layer begins. Then run `pnpm ccc:record` and `pnpm check:full` to verify.
