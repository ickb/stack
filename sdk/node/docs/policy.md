...
3. **Housekeeping rides along.** Every transaction also collects what is ready (receipts, then as many matured withdrawals as the DAO script's deposit-header index limit leaves room for after the receipts' headers, the rest next turn; a request's header past the limit fails the core as unfundable and the walk shortens the chain) and sweeps the bot's loose plain and iCKB cells, largest first, up to a 64 KiB prepared-size limit. Funding for the action comes first and may draw on collectible cells. Position in the transaction is protocol-fixed: the aligned action prefix first (deposit input i to request output i, matched order cells), everything else appended.

4. **The reserve is a sizing line, not a check.** Matches and deposits are planned to keep 1,000 CKB in liquid cells, plain and iCKB alike, since completion spends both; completion draws on that reserve by a bounded amount (a fee of at most the size budget times the fee rate, under 3 CKB at testnet's current rate; the iCKB change cell's capacity when the bot held no iCKB cell to sweep in; the deposit's receipt), and the next turn plans from the real balance again, so nothing compounds. Withdrawal owner markers are not bounded this way: a chain is sized in iCKB, so its markers may spend plain CKB down to fee headroom, as the tradeoffs below state. Nothing checks the completed transaction against the reserve: such a check rejected every fill sized to the reserve on testnet and could only ever stop the bot (amendment 52(i)). No other arithmetic bound.

5. **If the transaction does not complete,** try the next core: an exhausted deposit core continues with the withdrawal chain, shortest prefix last. No other chain is tried: a shorter prefix needs fewer markers and less iCKB, and skipping the oldest deposit instead would only help when the size-bounded sweep cannot reach iCKB the projection counted, which the next sent transaction consolidates. Collections and the sweep stay on every candidate: a matured withdrawal brings CKB in, a receipt becomes an iCKB cell of about its own capacity, neither adds a DAO output, and the sweep only adds inputs under the size budget, so none of them can make a fundable core unfundable (amendment 52(j)). A transaction sends only if it still carries a match, a deposit, a withdrawal request, or a collection. Compaction alone never sends. The profit gate applies to the match core only; housekeeping fees never veto a match.
