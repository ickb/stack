...
4. **The reserve is a sizing line, not a check.** Matches and deposits are planned to keep 1,000 CKB in plain cells; completion's fee and marker cells draw on that reserve by a bounded amount (a fee of at most the size budget times the fee rate, under 3 CKB at testnet's current rate; one iCKB cell; one receipt), and the next turn plans from the real balance again, so nothing compounds. Nothing checks the completed transaction against the reserve: such a check rejected every fill sized to the reserve on testnet and could only ever stop the bot (amendment 52(i)). No other arithmetic bound.
