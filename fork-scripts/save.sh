...
EXISTING=$(count_glob "$PINS_DIR"/local-*.patch)
...
git -C "$REPO_DIR" diff --cached "$PINNED_HEAD" > "$PINS_DIR/${PATCH_NAME}.patch"

# Verify patch is non-empty
if [ ! -s "$PINS_DIR/${PATCH_NAME}.patch" ]; then
...
# Update HEAD
git -C "$REPO_DIR" rev-parse HEAD > "$PINS_DIR/HEAD"

echo "Saved ${PATCH_NAME}.patch. Commit pins/ to share."
