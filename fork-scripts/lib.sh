...
# Reads all *-fork/config.json files and replaces the section between
...
sync_workspace_yaml() {
  ...
  for dev_dir in "$ROOT_DIR"/*-fork; do
    [ -f "$dev_dir/config.json" ] || continue
    local name clone_dir
    name=$(basename "$dev_dir")
    clone_dir=$(config_val "$dev_dir" '.cloneDir')

    mapfile -t includes < <(config_val "$dev_dir" '.workspace.include // [] | .[]')
    for inc in "${includes[@]}"; do
      entries+="  - ${name}/${clone_dir}/${inc}"$'\n'
    done

    mapfile -t excludes < <(config_val "$dev_dir" '.workspace.exclude // [] | .[]')
    for excl in "${excludes[@]}"; do
      entries+="  - \"!${name}/${clone_dir}/${excl}\""$'\n'
    done
  done

  awk -v entries="$entries" '
    /^  # @generated begin fork-workspaces/ { print; printf "%s", entries; skip=1; next }
    /^  # @generated end fork-workspaces/ { skip=0; print; next }
    !skip { print }
