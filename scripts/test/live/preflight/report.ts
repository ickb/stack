...
void test("preflight run reports public evidence for a generated unfunded key", async () => {
  ...
  const report = await runPreflight({
    env: configEnv({ chain: "testnet", privateKey, rpcUrl: rawRpcUrl }),
    prefix: "BOT",
    dependencies: mockDependencies({ userOrders, projectCalls }),
  });
...
void test("preflight reports CKB reserve and spendable balance separately", async () => {
  ...
  const report = await runPreflight({
    env: configEnv(baseConfig()),
    prefix: "BOT",
    dependencies: mockDependencies({ plainCkbBalance: 150000000000n, projection }),
  });
...
void test("preflight reports shared reserve and spendable balance separately", async () => {
  ...
  const report = await runPreflight({
    env: configEnv(baseConfig()),
    prefix: "BOT",
    dependencies: mockDependencies({ plainCkbBalance: 250000000000n, projection }),
  });
...
void test("preflight run uses the required explicit RPC URL", async () => {
  ...
  const report = await runPreflight({
    env: configEnv(baseConfig()),
    prefix: "BOT",
    dependencies: mockDependencies({ createPublicClientCalls: calls }),
  });
