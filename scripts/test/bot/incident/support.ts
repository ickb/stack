...
type CollectorResult = Awaited<ReturnType<typeof collectIncident>>;
...
const launcherChildExited = "launcher.child.exited";
...
function artifactEvent(timestamp: string, hash: string, artifactPath: string): string {
...
function parseIncidentSummary(text: string): IncidentSummary {
...
function parseVersionJson(text: string): VersionJson {
  const value: unknown = JSON.parse(text);
...
function isVersionJson(value: unknown): value is VersionJson {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
