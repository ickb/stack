...
export function botEvent(label: string, fields: Record<string, unknown> = {}): string {
...
export function writeBotEventScript(
  label: string,
  fields: Record<string, unknown> = {},
): string {
  return `process.stdout.write(${JSON.stringify(botEvent(label, fields))});`;
}
...
export interface LaunchRecord extends Record<string, unknown> {
  ...
  identity?: {
    bootId?: unknown;
    child?: { pid?: unknown; startTimeTicks?: unknown };
    launcher?: { pid?: unknown; startTimeTicks?: unknown };
  };
  logDir?: unknown;
  logFiles?: unknown;
  logRoot?: unknown;
...
export class FixtureChild extends EventEmitter {
  ...
  public killed = false;
  public lastSignal: NodeJS.Signals | number | undefined;
...
export async function processIdentityFixture(
  ...
  return { bootId: "test-boot", startTimeTicks: String(pid * 10) };
}
...
function parseLaunchRecord(line: string): LaunchRecord {
