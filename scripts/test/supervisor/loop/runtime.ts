...
void test("supervisor loop runs bounded supervisor commands until stable", async () => {
  const root = "/repo";
  ...
  try {
    ...
    assert.equal(
      firstSupervisor.options.timeout,
      DEFAULT_CHILD_TIMEOUT_SECONDS_VALUE * 1000,
    );
...
void test("supervisor loop prebuilds runtime before launching supervisor", async () => {
  const root = "/repo";
  const originalPrivateKey = process.env["PRIVATE_KEY"];
  process.env["PRIVATE_KEY"] = "operator-secret";
  const commands: BoundedCommandInvocation[] = [];
  ...
  try {
    ...
    for (const command of commands.slice(0, 1)) {
      ...
      assert.equal(
        ...
        DEFAULT_PREBUILD_TIMEOUT_SECONDS_VALUE * 1000,
      );
...
void test("supervisor loop reports prebuild failures without child output", async () => {
  const output = testOutput();
  const exitCode = await runSupervisorLoop({
...
void test("supervisor loop reports timed out prebuild failures", async () => {
  const output = testOutput();
  const exitCode = await runSupervisorLoop({
...
void test("supervisor loop accepts validation session out roots", async () => {
  const root = "/repo";
  await assertValidationOutRootAccepted({
    ...
    summaryPath: join(
      root,
      "log/validation/dynamic-test/chunks/chunk-0001/run-0001/summary.json",
    ),
  });
});

void test("supervisor loop rejects validation out roots outside chunk directories", async () => {
  const output = testOutput();
  const exitCode = await runSupervisorLoop({
...
void test("supervisor loop rejects validation chunk root descendants", async () => {
  const output = testOutput();
  const exitCode = await runSupervisorLoop({
...
void test("supervisor loop accepts explicit validation roots outside the repo", async () => {
  ...
  try {
    await assertValidationOutRootAccepted({
      ...
      outRoot,
      root: "/repo",
      summaryPath: `${outRoot}/run-0001/summary.json`,
    });
  } finally {
