...
describe(`${FRESH_MATCHABLE_ORDER_SKIP} creator provenance`, () => {
  it("protects every fresh actionable order returned by the owned-order scan", async () => {
    ...
    await expect(
      freshMatchableOrderSkip(
        ...
        [await matchableOrder(txHash)],
        headerLike({ number: 100180n, epoch: ccc.Epoch.from([0n, 0n, 1n]) }),
        { feeRate: 0n, ownedTxHash: byte32FromByte("4c") },
      ),
    ).resolves.toMatchObject({ reason: FRESH_MATCHABLE_ORDER_REASON, txHash });
    expect(getTransactionResponse).toHaveBeenCalledWith(txHash);
    expect(getTransaction).toHaveBeenCalledWith(txHash);
  ...
  it("prefers the owned origin after another qualifying fresh mint", async () => {
    const otherTxHash = byte32FromByte("4d");
    const ownedTxHash = byte32FromByte("4e");
    const { runtime } = freshOrderRuntime({ rpcBlockNumber: 100000n });

    await expect(
      freshMatchableOrderSkip(
        runtime,
        [await matchableOrder(otherTxHash), await matchableOrder(ownedTxHash)],
        headerLike({ number: 100180n, epoch: ccc.Epoch.from([0n, 0n, 1n]) }),
        { feeRate: 0n, ownedTxHash },
      ),
