...
- The SDK gains an immutable `Snapshot` API, typed planning results, branded money values, stable error codes, and an explicit transaction lifecycle.
- Bot and interface share one best-effort committed-cell compaction rule and persist no pending transaction identity.
...
| 2. SDK reshape      | Next     | `Snapshot`, typed results and errors, entity rewrite, exact committed-cell compaction, hybrid completion, dependency resolution, congruence, and debugger gates. |
...
These are the rewrite's fund-safety requirements. Some already exist in the code; exact input ownership and hybrid completion still need to be implemented.
...
- Post-deadline interface wording for the current bounded observation window.
