...
| 5. Depth            | Active   | Delivered: the destination field (amendment 52(af)), one editable address whose lock owns every cell the next transaction creates, so any transaction to another address is also a full move. Open: global liquidity and APY on the landing page if the visuals slice wants them; the position list, position dates, order cancellation, fee-safe CKB Max, and conversion history were deleted from scope in amendment 52(z). Test depth (real-header fixtures, mutation spot checks, live smoke wiring) struck in amendment 52(ae): the live testnet units are the real-header and smoke evidence. |
...
- Nothing from the review work: the questions once collected in [findings to investigate](findings-to-investigate.md) are closed in amendments 52(z), 52(ad), 52(ae), and 52(af); the file keeps the evidence pins.
