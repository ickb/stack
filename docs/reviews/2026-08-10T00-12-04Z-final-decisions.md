...
12. **Treeshake probe verdict (Phase-1 risk discovery, completed 2026-08-10)**: the single-entry/no-subpaths shape is safe — a synthetic merged barrel of all five packages bundles byte-identically to the per-package entry under rollup and esbuild; module-level `sideEffects:false` pruning works. Constraints adopted for Phase 2/3: (a) **entity modules host nothing but the entity** — hot leaf functions/constants live in side-effect-free modules (today's `core/src/udt.ts` mixing is the anti-pattern; ickb-attributable retention there is ~19-26KB); (b) **`/* @__PURE__ */` inside `static {}` blocks is banned** (lint rule): rollup deletes the codec application while keeping the class — silently runtime-broken entities; (c) the PURE-IIFE variant is a costed fallback only (changes the d.ts/api shape against §1/§8 — not adopted); (d) the mandatory Phase-3a packed-artifact probe asserts a ≤2KB budget on a one-symbol import with externalized CCC (size budget, not identifier denylist; esbuild pinned as devDependency). CCC upstream reports added to the amendment-11 pipeline: `dist/package.json` missing `sideEffects` (one-line, esbuild-verified 1,344,350→64 bytes), and the getter-namespace monolith (~1.3MB on any real `ccc.*` use, both layouts equally; external consumers additionally pay the unpatched joy-id chain).
