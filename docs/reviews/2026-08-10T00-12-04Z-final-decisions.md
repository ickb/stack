...
The integration audit documents **59 confirmed CCC defects** (rejected-tx/pending-cache correctness, empty-data cache poison 053-055, DAO claim epoch delayed one cycle 057, ACP discovery undercount 056, connector lifecycle races) against a CCC master pin that differs from the installed 1.17.0. Constraints: the rewrite **pins a CCC version and maps the audit against that pin**; the mapping (agent in flight; lands as an appendix here) is **blocking for the SDK send/wait/cache implementation phases** specifically — other phases may proceed.
